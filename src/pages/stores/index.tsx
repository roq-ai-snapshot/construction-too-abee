...
import { useRoqClient, useStoreFindManyWithCount } from 'lib/roq';
...
export function StoreListPage(props: StoreListPageProps) {
  ...
  const queryParams = useMemo(
    () =>
      convertQueryToPrismaUtil(
        {
          relations: ['organization', 'inventory.count', 'tool.count'],
          searchTerm: params.searchTerm,
          searchTermKeys: ['name.contains', 'location.contains', 'contact_number.contains'],
          ...params.filters,
        },
        'store',
      ),
  ...
  const { data, error, isLoading, mutate } = useStoreFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.store.delete({ where: { id } });
      await mutate();
    } catch (error) {
  ...
  const columns: ColumnType[] = [
    ...
    { id: 'location', header: 'Location', accessorKey: 'location' },
    {
      ...
      cell: ({ row: { original: record } }: any) =>
        record?.opening_time ? format(record?.opening_time, 'dd-MM-yyyy') : '',
    },
    {
      id: 'closing_time',
      header: 'Closing Time',
      ...
      cell: ({ row: { original: record } }: any) =>
        record?.closing_time ? format(record?.closing_time, 'dd-MM-yyyy') : '',
    },
