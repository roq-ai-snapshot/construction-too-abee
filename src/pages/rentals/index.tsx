...
import { useRoqClient, useRentalFindManyWithCount } from 'lib/roq';
...
export function RentalListPage(props: RentalListPageProps) {
      ...
      convertQueryToPrismaUtil(
        {
          ...
          searchTermKeys: ['payment_status.contains'],
          ...params.filters,
        },
        'rental',
      ),
  ...
  const { data, error, isLoading, mutate } = useRentalFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.rental.delete({ where: { id } });
      await mutate();
    } catch (error) {
  ...
  const columns: ColumnType[] = [
    {
      ...
      header: 'Rental Start Time',
      accessorKey: 'rental_start_time',
      cell: ({ row: { original: record } }: any) =>
        record?.rental_start_time ? format(record?.rental_start_time, 'dd-MM-yyyy') : '',
    },
    {
      id: 'rental_end_time',
      header: 'Rental End Time',
      accessorKey: 'rental_end_time',
      cell: ({ row: { original: record } }: any) =>
        record?.rental_end_time ? format(record?.rental_end_time, 'dd-MM-yyyy') : '',
    },
    { id: 'total_price', header: 'total_price', accessorKey: 'total_price' },
    { id: 'payment_status', header: 'Payment Status', accessorKey: 'payment_status' },
