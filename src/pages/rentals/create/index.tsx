...
function RentalCreatePage() {
  ...
  const formik = useFormik<RentalInterface>({
    initialValues: {
      rental_start_time: new Date(new Date().toDateString()),
      rental_end_time: new Date(new Date().toDateString()),
      total_price: 0,
      payment_status: '',
    ...
    >
      <Box rounded="md">
        ...
        <FormWrapper onSubmit={formik.handleSubmit}>
          <FormControl id="rental_start_time" mb="4">
            <FormLabel fontSize="1rem" fontWeight={600}>
              Rental Start Time
            </FormLabel>
            <DatePicker
              selected={formik.values?.rental_start_time ? new Date(formik.values?.rental_start_time) : null}
              onChange={(value: Date) => formik.setFieldValue('rental_start_time', value)}
            />
          </FormControl>
          <FormControl id="rental_end_time" mb="4">
            <FormLabel fontSize="1rem" fontWeight={600}>
              Rental End Time
            </FormLabel>
            <DatePicker
              selected={formik.values?.rental_end_time ? new Date(formik.values?.rental_end_time) : null}
              onChange={(value: Date) => formik.setFieldValue('rental_end_time', value)}
            />
          </FormControl>
