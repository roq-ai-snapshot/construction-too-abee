...
import { useRentalFindFirst, useRoqClient } from 'lib/roq';
...
function RentalViewPage() {
  ...
  const queryParams = useMemo<{ include: { user: boolean; tool: boolean } }>(
    () =>
  ...
  const { data, error, isLoading } = useRentalFindFirst(queryParams, {}, { disabled: !id });
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              ...
              >
                <FormListItem
                  ...
                  text={data?.rental_start_time ? format(data?.rental_start_time, 'dd-MM-yyyy') : ''}
                />

                <FormListItem
                  label="Rental End Time"
                  text={data?.rental_end_time ? format(data?.rental_end_time, 'dd-MM-yyyy') : ''}
                />
