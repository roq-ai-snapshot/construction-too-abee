...
import { useRoqPlatformUserProfiles, useRentalCount, useStoreCount, useToolCount } from 'lib/roq';
...
function DashboardPage() {
  ...
  const rentalStat = useRentalCount();

  const storeStat = useStoreCount();

  const toolStat = useToolCount();

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Rentals',
      result: rentalStat?.data,
      isLoading: rentalStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Stores',
      result: storeStat?.data,
      isLoading: storeStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Tools',
      result: toolStat?.data,
