...
import { useRoqClient, useToolFindManyWithCount } from 'lib/roq';
...
export function ToolListPage(props: ToolListPageProps) {
  ...
  const queryParams = useMemo(
    () =>
      convertQueryToPrismaUtil(
        {
          relations: ['store', 'inventory.count', 'rental.count'],
          searchTerm: params.searchTerm,
          searchTermKeys: ['name.contains', 'type.contains', 'brand.contains'],
          ...params.filters,
        },
        'tool',
      ),
  ...
  const { data, error, isLoading, mutate } = useToolFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.tool.delete({ where: { id } });
      await mutate();
    } catch (error) {
