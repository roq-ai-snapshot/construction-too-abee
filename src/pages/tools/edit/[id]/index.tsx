...
import { useRoqClient, useToolFindFirst } from 'lib/roq';
...
function ToolEditPage() {
  ...
  const { data, error, isLoading, mutate } = useToolFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: ToolInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.tool.update({
        data: values as RoqTypes.tool,
        where: {
          id,
        },
      });
